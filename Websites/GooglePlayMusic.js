...
function setup() {
  ...
  gpmInfoHandler.readyCheck = function() {
    ...
    return (
      ...
      !!document.getElementById("currently-playing-title").innerText.length &&
  ...
  gpmInfoHandler.album = function() {
    if (document.getElementsByClassName("player-album").length) {
  ...
  gpmInfoHandler.rating = function() {
    ...
    if (
      document.getElementsByClassName("rating-container materialThumbs")[0]
        .children[0].children[0].children[0].children[0].children.length < 2
    ) {
      return 5;
    }
    if (
      document.getElementsByClassName("rating-container materialThumbs")[0]
        .children[1].children[0].children[0].children[0].children.length < 2
    ) {
      return 1;
    }
  ...
  gpmInfoHandler.repeat = function() {
    if (
      document
        ...
        .children[1].className.includes("active")
    ) {
      //Hacky way to check if repeat all
      if (
        document
          .getElementsByClassName("material-player-middle")[0]
          .children[1].children[0].children[0].children[0].children[0].getAttribute(
            "d"
          ) ==
  ...
  gpmInfoHandler.shuffle = function() {
    if (
      document
        .getElementsByClassName("material-player-middle")[0]
        .children[5].className.includes("active")
    ) {
      return 1;
    }
    return 0;
  ...
  gpmEventHandler.toggleThumbsUp = function() {
    document
      .getElementsByClassName("rating-container materialThumbs")[0]
      .children[0].click();
  };

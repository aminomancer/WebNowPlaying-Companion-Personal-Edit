...
function setup() {
  ...
  twitchInfoHandler.readyCheck = function() {
    return (
      !!document.getElementsByClassName("video-player__default-player")
        .length && !!document.getElementsByTagName("video").length
    );
  };
  ...
  twitchInfoHandler.cover = function() {
    return document
      .querySelector(".tw-avatar--size-64 > img:nth-child(1)")
      .src.replace("70x70", "600x600");
  };
  ...
  twitchInfoHandler.rating = function() {
    if (
      document
        .getElementsByClassName("follow-btn__follow-btn")[0]
        .className.includes("following")
    ) {
      return 5;
    }
    return 0;
  ...
  twitchEventHandler.next = function() {
    ...
    if (document.getElementsByTagName("video")[0].duration != "1073741824") {
      document.getElementsByTagName(
        "video"
      )[0].currentTime = document.getElementsByTagName("video")[0].duration;
    }
  };

...
function setup() {
  ...
  amznInfoHandler.readyCheck = function () {
    if (
      document.getElementsByTagName("music-horizontal-item").length > 0 &&
      document.getElementById("transport") !== null
    ) {
      return document
        .getElementsByTagName("music-horizontal-item")
        [document.getElementsByTagName("music-horizontal-item").length - 1].getAttribute(
          "primary-text"
        ).length > 0
        ? 1
        : 0;
  ...
  amznInfoHandler.duration = function () {
    ...
    var timeFromBeginingString =
      document.getElementById("transport").lastElementChild.lastElementChild.children[0].innerText;
  ...
  amznInfoHandler.rating = function () {
    ...
    if (
      document
        .getElementById("transport")
        .children[0].children[1].children[0].children[0].getAttribute("icon-name") === "done"
    ) {
      return 5;
    }
    return 0;
  ...
  amznEventHandler.rating = function (rating) {
    if (rating > 3) {
      ...
      if (
        document
          .getElementById("transport")
          .children[0].children[1].children[0].children[0].getAttribute("icon-name") === "add"
      ) {
        document
          .getElementById("transport")

...
function setup() {
  ...
  amznInfoHandler.readyCheck = function() {
    if (
      document.getElementsByTagName("music-horizontal-item").length &&
      document.getElementById("transport") !== null
    ) {
      return document
        .getElementsByTagName("music-horizontal-item")
        [
          document.getElementsByTagName("music-horizontal-item").length - 1
        ].getAttribute("primary-text").length
        ? 1
        : 0;
    }
    return 0;
  ...
  amznInfoHandler.state = function() {
    ...
    return document
      ...
      .children[1].children[2].getAttribute("icon-name")
      .includes("pause")
      ? 1
      : 2;
  };
  amznInfoHandler.title = function() {
    return document
      .getElementById("transport")
      .children[0].children[0].getAttribute("primary-text");
  };
  amznInfoHandler.artist = function() {
    return document
      .getElementById("transport")
      .children[0].children[0].getAttribute("secondary-text");
  };
  amznInfoHandler.album = function() {
    return document
      .getElementById("transport")
      .children[0].children[0].getAttribute("secondary-text-2");
  };
  amznInfoHandler.cover = function() {
    return document
      .getElementById("transport")
      .children[0].children[0].getAttribute("image-src");
  };
  amznInfoHandler.duration = function() {
    ...
    var timeFromEndString = document
      .getElementById("transport")
      .lastElementChild.lastElementChild.children[1].innerText.replace(
        "- ",
        ""
      );
    var timeFromEndSec =
      parseInt(
        timeFromEndString.substring(timeFromEndString.indexOf(":") + 1)
      ) +
      parseInt(timeFromEndString.substring(0, timeFromEndString.indexOf(":"))) *
        60;
    var timeFromBeginingString = document.getElementById("transport")
      .lastElementChild.lastElementChild.children[0].innerText;
    var timeFromBeginingSec =
      parseInt(
        timeFromBeginingString.substring(
          timeFromBeginingString.indexOf(":") + 1
        )
      ) +
      parseInt(
        timeFromBeginingString.substring(0, timeFromBeginingString.indexOf(":"))
      ) *
        60;
  ...
  amznInfoHandler.rating = function() {
    ...
    if (
      document
        .getElementById("transport")
        .children[0].children[1].children[0].children[0].getAttribute(
          "icon-name"
        ) === "done"
    ) {
      return 5;
  ...
  amznInfoHandler.repeat = function() {
    var state = document
      .getElementById("transport")
      .children[1].children[0].getAttribute("aria-label");
  ...
  amznInfoHandler.shuffle = function() {
    ...
    if (
      document
        .getElementById("transport")
        .children[1].children[4].getAttribute("aria-label")
        .includes("Off")
    ) {
      return 1;
    }
  ...
  amznEventHandler.toggleThumbsUp = function() {
    document
      .getElementById("transport")
      .children[0].children[1].children[0].children[0].click();
  };

...
function setup() {
  ...
  pocketInfoHandler.readyCheck = function() {
    return (
      ...
      !!document.getElementsByClassName("player_episode").length
    );
  };
  ...
  pocketInfoHandler.cover = function() {
    var cover = document
      .getElementsByClassName("player_artwork")[0]
      .children[0].getAttribute("data-fallback-src");

    if (cover === null || !cover.length) {
      cover = document.getElementsByClassName("player_artwork")[0].children[0]
        .src;
    }

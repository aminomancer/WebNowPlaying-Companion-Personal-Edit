...
function setup() {
  ...
  yandexMusicEventHandler.progressSeconds = function(position) {
    ...
    scriptHTML.innerText =
      "externalAPI.setPosition(" +
      position +
      ');document.getElementsByClassName("embedded-script-progress")[0].remove();';
  ...
  yandexMusicEventHandler.volume = function(volume) {
    ...
    scriptHTML.innerText =
      "externalAPI.setVolume(" +
      volume +
      ');document.getElementsByClassName("embedded-script-volume")[0].remove();';
    document.head.appendChild(scriptHTML);
  };

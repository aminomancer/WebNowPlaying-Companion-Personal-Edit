...
function setup() {
  ...
  VIBEInfoHandler.readyCheck = function() {
    return !!document.getElementsByClassName("menu_item item_library").length;
  };

  VIBEInfoHandler.state = function() {
    return document
      .getElementsByClassName("btn_now")[0]
  ...
  VIBEInfoHandler.cover = function() {
    var currCover = document.getElementsByClassName("thumb_cover")[0]
      .children[0].src;
  ...
  VIBEInfoHandler.volume = function() {
    return (
      parseInt(
        document.getElementsByClassName("bar_status")[0].children[0].style.width
      ) / 100
    );
  };
  VIBEInfoHandler.rating = function() {
    if (
      document.getElementsByClassName("btn_like")[0].className.includes("on")
    ) {
      return 5;
    }
    return 0;
  ...
  VIBEInfoHandler.repeat = function() {
    if (
      document
        .getElementsByClassName("btn_repeat")[0]
        .className.includes("once")
    ) {
      return 2;
    }
    if (
      document.getElementsByClassName("btn_repeat")[0].className.includes("all")
    ) {
      return 1;
    }
    return 0;
  ...
  VIBEInfoHandler.shuffle = function() {
    if (
      document.getElementsByClassName("btn_shuffle")[0].className.includes("on")
    ) {
      return 1;
    }
    return 0;
  ...
  VIBEEventHandler.progress = function(progress) {
    var loc = document
      .getElementsByClassName("playing_progress")[0]
      .getBoundingClientRect();
    progress *= loc.width;

    var a = document.getElementsByClassName("playing_progress")[0].children[0]
      .children[1];
  ...
  VIBEEventHandler.volume = function(
    volume // kanged from Spotify.js
  ) {
    var loc = document
      .getElementsByClassName("bar_volume")[0]
      .getBoundingClientRect();
    volume *= loc.width;

    var a = document.getElementsByClassName("bar_volume")[0].children[0]
      .children[0];
    var e = document.createEvent("MouseEvents");
    e.initMouseEvent(
      "mousedown",
